...
while (vs.isOpened()):
	...
	#Video writing.
        ...
        ret,threshold = cv2.threshold(g,232,255,cv2.THRESH_BINARY)

        #Lower extra noise
        opening = cv2.morphologyEx(threshold, cv2.MORPH_DILATE,element)
        closing = cv2.morphologyEx(opening, cv2.MORPH_ERODE,element)

        #For threshold overlay during debugging
        if debug:
                threshold = cv2.merge((closing,closing,closing));
